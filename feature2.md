...
- Hybrid retrieval (vector + lexical over current document chunks, the accepted Product Brain, brain nodes, change proposals, decision records, dashboard snapshots, communication messages)
...
| Communication messages | Prisma `findMany` lexical only | `lex × commWeight` |
...
Citations are generated by Claude from the evidence pack. They are **not** backend-verified for existence (citations are descriptive, not navigable — open-targets are the navigable layer). The `refId` must be a UUID matching the `id` of a retrieved candidate.
...
- `message`: blocked entirely in client context. Must exist with matching `projectId`.
- `thread`: blocked entirely in client context. Must exist with matching `projectId`.
- `brain_node`: must exist with matching `projectId`. Allowed in client context.
- `change_proposal`: blocked in client context. Must exist with matching `projectId`.
- `decision_record`: blocked in client context. Must exist with matching `projectId`.
- `dashboard_filter`: always valid. No DB check needed.
...
| `tests/socrates-hybrid.test.ts` | Hybrid retrieval: parse-revision safety, Product Brain retrieval, neighbor parse revision |
...
| Item | Status | Notes |
|---|---|---|
| Communication message embeddings | Deferred | Messages use lexical-only retrieval. Embedding message_chunks for vector search is spec-documented but not implemented in this revision. |
...
| Streaming timeout | Deferred | No explicit timeout on the Anthropic stream. Long-running answers can hold the SSE connection indefinitely. A 90s client-side timeout is recommended. |
